...
// New type for updated backend response
export interface AvailabilityDateSlots {
  dateStr: string;
  slots: string[];
}

export interface AvailabilityDayWithDates {
  day: DayOfWeekType;
  dates: AvailabilityDateSlots[];
  isAvailable: boolean;
}

export interface PublicAvailabilityEventResponseWithDatesType {
  message: string;
  data: AvailabilityDayWithDates[];
}
