...
import { AvailabilityDayWithDates, AvailabilityDateSlots } from "@/types/api.type";
...
const BookingCalendar = ({
  ...
  const availability = (data?.data as AvailabilityDayWithDates[]) || [];

  // Get time slots for the selected date (updated for new backend response)
  let timeSlots: string[] = [];
  if (selectedDate) {
    const selectedDay = format(selectedDate.toDate(timezone), "EEEE").toUpperCase();
    const selectedDateStr = format(selectedDate.toDate(timezone), "yyyy-MM-dd");
    const dayObj = availability.find((day: AvailabilityDayWithDates) => day.day === selectedDay);
    const dateObj = dayObj?.dates?.find((date: AvailabilityDateSlots) => date.dateStr === selectedDateStr);
    timeSlots = dateObj?.slots || [];
  }
